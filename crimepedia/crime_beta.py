...
def newaccount():
	...
	def addnewinfo():
		...
		answer=tkinter.messagebox.askquestion("Continue?","Add New Account ?")
		if (answer=="yes"):
			id=[]
			with open("admin.txt","r") as f:
				for line in f:
					line=line.replace("\n",'')
					id.append(de_encryption(line))
			length1=len(id)
			...
			if(flag==1):
				...
				encrpyted_string+='\n'
				file.write(encrpyted_string)
				file.close()
				root.destroy()
	buttonok=Button(root,text="Done",height=1,width=10,command=addnewinfo,bg="#3D5AFE",fg="white",borderwidth=0)
